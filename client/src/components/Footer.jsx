...
const menuItems = {
  'Shop': ['Gift cards', 'Etsy blog'],
  'Sell': ['Sell on Etsy', 'Teams', 'Forums', 'Affiliates'],
  'About': ['Etsy, Inc.', 'Policies', 'Investors', 'Careers', 'Press', 'Impact'],
  'Help': ['Help Center', 'Privacy settings'],
};

const legal = ['Terms of Use', 'Privacy', 'Interest-based ad'];
...
class App extends React.Component {
  render() {
    return (
      <FooterContainer>
        <Cont>
          <EmailSvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 30" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
        ...
        <Cont1>
          <EmailContainer>
            <EmailSignUp>Yes! Send me exclusive offers, unique gift ideas, and personalized tips for shopping and selling on Etsy.</EmailSignUp>
            <EmailInput placeholder="Enter your email"></EmailInput>
          </EmailContainer>
        </Cont1>
        <Cont1>
          <Cont>
          <MenuSvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 30" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
          ...
          <MenuListContainer>
            {Object.keys(menuItems).map((key, index) => {
              return (
                <MenuList key={index}>
                  <Header>{key}</Header>
                  {menuItems[key].map((value, index) => {
                    return (
                      <MenuListItem key={index}>
                        <MenuListItemLink href="#">{value}</MenuListItemLink>
                      </MenuListItem>
                    );
                  })}
                </MenuList>
        ...
        <Cont3>
          <Copyright>
            <Country>United States &nbsp; | &nbsp; English (US) &nbsp; | &nbsp; $ (USD)</Country>
            <BottomMenu>
              <BottomMenuText>© 2020 Etsy, Inc.</BottomMenuText>
              <BottomMenuList>
                {legal.map((name, index) => {
                  return (
                    <BottomMenuListItem key={index}><BottomMenuListItemLink>{name}</BottomMenuListItemLink></BottomMenuListItem>
                  )
                })}
              </BottomMenuList>
            </BottomMenu>
